...
#define DEBUG_SEND_PACKETS 1 //when this is set to 1 will print SEND packets on console.
#define DEBUG_RECV_PACKETS 1 //when this is set to 1 will print RECV packets on console.
...
struct SCRTY_1_STEP_STRUCT;
struct SCRTY1_FILE_STRUCT;
...
struct BMP_FILE_STRUCT
{
    ...
    /* 0x108/264 */ unsigned char *BMPPointer;
...
struct SCRTY_1_STEP_STRUCT
{
    /* 0/0 */ unsigned int Key;
    /* 0x4/4 */ unsigned int ExpandedKey[20];
};

struct SCRTY1_FILE_STRUCT
{
    /* 0/0 */ unsigned int Version;
    /* 0x4/4 */ struct SCRTY_1_STEP_STRUCT Step1[1024];
    /* 0x15004/86020 */ struct SCRTY_1_STEP_STRUCT Step1And2[1024];
};
...
struct CONNECTION
{
    ...
    /* Continuum Expanded Key */ unsigned int ContinuumExpandedKey[20]; //added
    /* Continuum KeyExpansionOffset */ unsigned short ContinuumKeyExpansionOffset;
...
// sizeof(struct SECURITY_STRUCT) = 4036 + 172036 for scrty1
struct SECURITY_STRUCT
{
    ...
    /* 0xFC4/4036 */  struct SCRTY1_FILE_STRUCT Scrty1File;
    /* Continuum Checksum */ int ContinuumEXEChecksum;
};
// sizeof(PACKET) = 14
struct PACKET
{
...
struct PLAYER
{
    ...
    /* 0x18/24 */ unsigned int SubspaceEXEChecksumIndex; //REMOVED
...
struct ARENA {
    ...
    /* 0xFF34/65332 */ unsigned int CurrentScrtyDataIndex;
...
unsigned char CRC8LookupTable[256] =
{
...
signed int __cdecl IsBannedIPAddress(const char *a2);
...
unsigned int ROR(unsigned int var, unsigned int amount);
void ContinuumEncryptionMD5Step3(unsigned int *ArrayOfIntegers, unsigned int a2);
void ContinuumDecrypt(struct CONNECTION *connection, unsigned char *buffer, unsigned int *packetlen);
void ContinuumEncrypt(struct CONNECTION *connection, unsigned char *buffer, unsigned int *packetlen);
unsigned char Crc8(unsigned char *packet, int length);
...
////////////////////
    /*
        unsigned char bytes[80] = {0x6C, 0xA4, 0x27, 0xF2, 0xA4, 0x9B, 0xFA, 0x5B, 0xC4, 0xA2, 0x39, 0xB0, 0xD4, 0x3A, 0xA3, 0x7B, 0xC8, 0x9C, 0xAD, 0x52, 0x8D, 0x8A, 0x51, 0xED, 0xC7, 0x8F, 0x0A, 0xA8, 0x1F, 0xE9, 0xFB, 0x24, 0x11, 0xAB, 0xA3, 0x33, 0x52, 0xA4, 0x4E, 0x10, 0xBE, 0x7F, 0x1B, 0xFB, 0xC8, 0x93, 0xCB, 0xD6, 0x93, 0x70, 0x53, 0xF0, 0x34, 0x6A, 0x52, 0x01, 0x46, 0x06, 0xD0, 0x7E, 0x8E, 0x61, 0x65, 0xD4, 0x26, 0xAF, 0xA6, 0x13, 0x2B, 0xDD, 0x08, 0x24, 0x6D, 0x68, 0x05, 0x02, 0x86, 0x5E, 0x42, 0xEF};
        unsigned char packet[172] = { };
        printf("BEFORE BYTES\n");
        int b = 0;
        for(b = 0; b < 172; b++) {
            printf("%X ", packet[b]);
        ...
        memcpy(&test.ContinuumExpandedKey, bytes, 80);
        int size = 172;
        ContinuumDecrypt(&test, packet, &size);
        ContinuumEncrypt(&test, packet, &size);
        printf("AFTER BYTES\n");
        int c = 0;
        for(c = 0; c < size; c++) {
            printf("%X ", packet[c]);
...
void __cdecl PlayerEntering(struct ARENA *arena, struct PLAYER *player)
{
    ...
    char MapInformationPacket[25]; // [sp+88h] [bp-53C8h]@91
...
SCORE_NOT_FOUND:
    ...
    strncpy(&MapInformationPacket[1], arena->MapFilename, 0x10);
    *(unsigned int *)&MapInformationPacket[17] = arena->MapChecksum;
    *(unsigned int *)&MapInformationPacket[21] = arena->MapFileSize;
    SendPlayerReliablePacket(player, MapInformationPacket, 25, 1);
...
bool __cdecl ProcessArena(struct ARENA *arena)
{
    ...
    if ( (GetTickCount() / 0xA - arena->LastArenaSettingsUpdateTime) > 6000 || arena->NeedSettingsUpdate )
    {
        ...
        if ( arena->SecurityChecksumCounterOddValue++ & 1 ) //only works every other time, lol.
        {
            ...
            arena->CurrentScrtyDataIndex = rand() % 500; // Random value from Scrty File
            v50 = *(unsigned int *)&Security.ScrtyData[8 * arena->CurrentScrtyDataIndex];
...
void __cdecl LoadBMPHeader(struct BMP_FILE_STRUCT *BMPFileStruct, const char *Source)
{
    ...
    if ( fileHandle )
    {
        ...
        BMPFileStruct->BMPPointer = (char *)AllocateMemory(4 * BMPFileStruct->TotalMapTiles);
        fread(BMPFileStruct->BMPPointer, 4, BMPFileStruct->TotalMapTiles, fileHandle);
        fclose(fileHandle);
    }
    ...
    if ( BMPFileStruct->field_104 )
    {
        if ( BMPFileStruct->BMPPointer )
        {
            ...
            if ( v2 || (v2 = fopen(BMPFileStruct->FileName, "wb")) != 0 )
            {
                ...
                fwrite(BMPFileStruct->BMPPointer, BMPFileStruct->TotalMapTiles, 4, v2);
...
BOOL __cdecl IsFileLastWrittenTime(char * lpFileName, unsigned int *a2)
{
    ...
    v2 = (unsigned int)CreateFileA(lpFileName, GENERIC_READ, 0, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
    v3 = (void *)v2;
    if ( v2 != -1 )
    {
...
void __cdecl PlayerHandleGamePacket(struct PLAYER *player, unsigned char *packet, int packetSize)
{
    ...
    int v61; // ecx@92
    int v62; // eax@92
    signed int OldScrtyMatchFound; // esi@93
    unsigned int v64; // eax@93
    ...
    case 0x22:                                    // 0x22 - Security Violation Unknown
        if ( player->MyArena && !player->IsSuperModerator )
        {
            ...
            {
                if ( *(unsigned int *)&Security.ScrtyData[4] != *(unsigned int *)&packet[9] )
                {
        ...
        if ( player->LastSecurityPacketReturnedTime )
        {
            ...
            else
            {
                if ( player->MyArena && !player->IsSuperModerator && player->MyArena->SettingsChecksum )
                {
                    ...
                    if ( *(unsigned int *)&Security.ScrtyData[8 * player->MyArena->CurrentScrtyDataIndex + 4]  != v61 )
                    {
                        OldScrtyMatchFound = 0;
                        v64 = 4;
                        do
                        {
                            if ( *(unsigned int *)&Security.ScrtyData[v64] == v61 )
                                OldScrtyMatchFound = 1;
                            v64 += 8;
                        }
                        while ( v64 < 500 );
                        if ( OldScrtyMatchFound )
    ...
    case 0x0B:                                    // 0x0B - SSUpdate.EXE Request
        if ( player->SubspaceVersion == 132
                && Security.Subspace132EXEBytes != 0 ) {
            //Send UPDATE2.EXE aka Subspace 1.32 installer
            SendHugeChunkPacketNotAllocated(
                player->connection,
                Security.Subspace132EXEBytes,
                Security.Subspace132FileSize,
                0);
        } else if ( player->SubspaceVersion == 133
                    && Security.Subspace133EXEBytes != 0 ) {
            //Send UPDATE1.EXE aka Subspace 1.33 installer
            SendHugeChunkPacketNotAllocated(
                player->connection,
                Security.Subspace133EXEBytes,
                Security.Subspace133FileSize,
                0);
        } else {
            //Send UPDATE.EXE aka Subspace 1.31 installer
            SendHugeChunkPacketNotAllocated(
                player->connection,
                Security.Subspace131EXEBytes,
                Security.Subspace131FileSize,
                0);
        }
        return;
...
LABEL_432:
    ...
    case 0x01:                                     // 0x01 - Arena login
        ...
        {
            if ( player->IsSuperModerator
                    || GetProtocolVersion(player->connection) == 1
                    || GetProtocolVersion(player->connection) == 0x11 )
            {
...
LABEL_606:
    ...
    case 0x24:					// 0x24 - Password packet Continuum
        player->DemoPlayer = 0;
...
LABEL_621:
        ...
        *(unsigned int *)&player->SubspaceVersion = *(unsigned short *)&packet[75];
        //A little trick to detect Continuum, lol.
        if(player->SubspaceVersion >= 40 && player->SubspaceVersion <= 100 ) {
            unsigned char ContinuumVersionPacket[7];
            ContinuumVersionPacket[0] = 0x34;
            *(unsigned short *)&ContinuumVersionPacket[1] = player->SubspaceVersion;
            //TODO: make a custom ContinuumEXEChecksum that will work with Continuum.exe
            *(unsigned int *)&ContinuumVersionPacket[3] = Security.ContinuumEXEChecksum;
            SendPlayerReliablePacket(player, ContinuumVersionPacket, 7, 1);
        }

        if ( !Security.SubspaceEXEChecksum )
        {
        ...
        if ( IsBannedIPAddress(GetIPAddressString(player->IPAddressDWORD)) )
        {
            v344[0] = 0x0A;                         // 0x0A - Password Packet Response
        ...
        else
        {
            if ( IsLineExistTextFile(ReservedPointer, nameReturn) < 0 || a2 )
            {
                ...
                *(unsigned int *)&v344[14] = 0;             // ? Unknown
                v344[18] = 0;                         // ? Unknown
                v344[19] = 0;                         // Registration Form Request (Boolean)
                *(unsigned int *)&v344[20] = *(unsigned int *)&Security.ScrtyData[4];
                if(player->SubspaceVersion >= 40 && player->SubspaceVersion <= 100)
                    *(unsigned int *)&v344[10] = Security.ContinuumEXEChecksum;// Continuum.exe Checksum
                else
                    *(unsigned int *)&v344[10] = Security.SubspaceEXEChecksum;// Subspace.exe Checksum
...
void __cdecl ServerInitialize()
{
    ...
    int v3; // ebp@15
    struct SECURITY_STRUCT *v4; // eax@15
    int v5; // edi@15
...
//TODO: version0 went up to version9 for up to 9 different revisions of the game to connect?
//I've removed version system completely, Security array which was 10 elements is no longer a array.

    Security.Subspace131EXEBytes = 0;
    Security.Subspace133EXEBytes = 0;
    Security.Subspace132EXEBytes = 0;

    strcpy(cp, "Update.exe");
    if ( _access(cp, 0) != -1 )
        Security.Subspace131EXEBytes = CompressFile(cp, &Security.Subspace131FileSize, &LocalPointer, Buffer, 0x11, 0);

    strcpy(cp, "Update1.exe");
    if ( _access(cp, 0) != -1 )
        Security.Subspace133EXEBytes = CompressFile(cp, &Security.Subspace133FileSize, &LocalPointer, Buffer, 0x11, 0);

    strcpy(cp, "Update2.exe");
    if ( _access(cp, 0) != -1 )
        Security.Subspace132EXEBytes = CompressFile(cp, &Security.Subspace132FileSize, &LocalPointer, Buffer, 0x11, 0);

    strcpy(cp, "subspace.exe");
    if ( _access(cp, 0) != -1 )
    {
        Security.SubspaceEXEChecksum = SubspaceEXEChecksum(cp);
        ...
        if ( v34 )
        {
            ...
            fread(Security.ScrtyData, 1, 4000, v34);
            fclose(v34);
        }
    }

    strcpy(cp, "continuum.exe");
    if ( _access(cp, 0) != -1 )
    {
        Security.ContinuumEXEChecksum = SubspaceEXEChecksum(cp);
        strcat(cp, " Z"); //subspace.exe Z or continuum.exe Z creates scrty file.
        system(cp); //creates and opens new process (starts subspace.exe Z, can't run this on linux anyways -_-.
        ...
        //Continuum Security File Scrty1
        v34 = fopen("scrty1", "rb");
        if ( v34 )
        {
            v36 = _fileno(v34);
            if ( _filelength(v36) != 172036 )
            {
                printf("Security file 2 invalid size\n");
                exit(1);
            }
            fread(&Security.Scrty1File, 1, 172036, v34);
            printf("Loaded support for Continuum v0.%d\n", Security.Scrty1File.Version);
            fclose(v34);
        } else {
            printf("No support for Continuum loaded (Missing scrty1 file))\n");
        }
    }

    strcpy(cp, "ipblock.txt");
    Security.IPBlockText = malloc (sizeof (struct TEXT_FILE_STRUCT));
    if ( Security.IPBlockText )
        InitializeTextFile(Security.IPBlockText, cp, 1);
    else
        Security.IPBlockText = 0;

    strcpy(cp, "ipallow.txt");
    Security.IPAllowText = malloc (sizeof (struct TEXT_FILE_STRUCT));
    if ( Security.IPAllowText )
        InitializeTextFile(Security.IPAllowText, cp, 1);
    else
        Security.IPAllowText = 0;
                    ...
                    do
                    {
                        if ( sub_40CF10(ZonePlayers[v1]) )
                        {
                            ...
                            memcpy(
                                &ZonePlayers[v1],
                                &ZonePlayers[v1 + 1],
                                sizeof(struct PLAYER *) * (ZonePlayerCount - v1 - 1));
                            --ZonePlayerCount;
                            --v1;
                        }
                        ++v1;
...
SKIP_KICKED_USERS:
            ...
            if ( (GetTickCount() / 0xA - LastFilesReReadAndBillerConnectionCheckTime) > 18000 || IsEditedServerINIorCFG != 0 )
            {
                ...
                if ( Security.IPBlockText != 0 )
                    LoadTextFile(Security.IPBlockText, 0);
                if ( Security.IPAllowText != 0 )
                    LoadTextFile(Security.IPAllowText, 0);
...
void __cdecl ServerUninitialize()
{
    ...
    if ( ModeratePointer )
    {
        ...
        free(ModeratePointer);
    }
    if ( ReservedPointer )
    {
        CleanTextFileMemory(ReservedPointer);
        free(ReservedPointer);
    }
    if ( IDBlockPointer )
    {
    ...
    if ( Security.IPBlockText )
    {
        CleanTextFileMemory(Security.IPBlockText);
        free(Security.IPBlockText);
    }

    if ( Security.IPAllowText )
    {
        CleanTextFileMemory(Security.IPAllowText);
        free(Security.IPAllowText);
    }
    ...
    case 1:
        ...
        if ( ZonePlayerCount > 0 )
        {
            ...
            *(unsigned int *)&buf[24] = NewsTxtFileChecksum;// News.txt Checksum
            if(ZonePlayers[v11]->SubspaceVersion >= 40 && ZonePlayers[v11]->SubspaceVersion <= 100)
                *(unsigned int *)&buf[10] = Security.ContinuumEXEChecksum;// Continuum.exe Checksum
            else
                *(unsigned int *)&buf[10] = Security.SubspaceEXEChecksum;// Subspace.exe Checksum
            *(unsigned int *)&buf[20] = *(unsigned int *)&Security.ScrtyData[4];
            if ( ZonePlayers[v11]->IsModerator )
            {
...
signed int __cdecl IsBannedIPAddress(const char *a2)
{
    ...
    {
        TotalLines = TextFileTotalLines(Security.IPAllowText);

        if ( TotalLines > 0 )
        {
            while ( 1 )
            {
                v8 = TextFileByIndex(Security.IPBlockText, CurrentLines);
                if ( v5 >= strlen(v8) )
                    v9 = strlen(v8);
                else
                    v9 = v5;
...
LABEL_38:
        if ( Security.IPBlockText == 0
                ...
                || (CurrentLinesTwo = 0, TotalLinesTwo = TextFileTotalLines(Security.IPBlockText), TotalLinesTwo <= 0) )
        {
            result = 0;
        }
        else
        {
            while ( 1 )
            {
                v18 = TextFileByIndex(Security.IPBlockText, CurrentLinesTwo);
...
LABEL_36:
    if ( *ChatText == '*' && player ) //All staff commands go in this massive if statement
    {
        ...
        if( player->IsModerator ) {
            ...
            else if ( !memicmp(ChatText, "*endlog", 7) )
            {
                if ( !player )
                    return;
...
void __cdecl PlayerHandlePacket(struct PACKET_ATTACHMENT *packetAttachment, unsigned char *buffer, signed int RecvLength, struct in_addr RemoteIP, unsigned short RemotePort)
{
    ...
    bool found; //fix 1
    unsigned char Step1ContinuumResponseBuffer[10];
    unsigned char Step1And2ContinuumResponseBuffer[12];
    ...
    //Start decrypting as long as it's not the first packet 0x01 - Encryption Request
    //Never decrypt if a packet starts with 0x00 Core packet header
    if ( buffer[0] || buffer[1] != 1 )
    {
        ...
        //Packets that get processed without decrypting.
        if ( !buffer[0] && (buffer[1] == 2 || buffer[1] == 0x11)) {
            ProcessCorePackets(connection, buffer, RecvLength, 0);
            return;
        }
        //Any packets less then 2 are not encrypted.
        if ( RecvLength < 2 ) {
            printf("!!!!!!!!!!!!!!!!!!!!!Found 1: %d", buffer[0]);
            return;
        }

        if(connection->ProtocolVersion == 1) { //SubSpace Decryption
            unsigned short StartOffset = 1;
            if ( !buffer[0] ) //If the byte at offset 0 is 0x00
                StartOffset = 2; //Skip Next Byte as its a Core PacketType
            v35 = connection->ServerKey;

            for(unsigned short Counter = 0; Counter < RecvLength; Counter += 4) {
                v37 = *(unsigned int *)&buffer[Counter+StartOffset];
                v38 = v35 ^ *(unsigned int *)&connection->SSEncryptionTable[Counter];
                *(unsigned int *)&buffer[Counter+StartOffset] = v37 ^ v38;
                v35 = v37;
            }
        } else if(connection->ProtocolVersion == 0x11 ) { //Continuum Step1And2 Decryption probably supports ProtocolVersion == 0x10 too.
            ContinuumDecrypt(connection, buffer, &RecvLength);
        }

#if (DEBUG_RECV_PACKETS == 1)
        printf("[RECV][Size: %d]: ", RecvLength);
        int i = 0;
        ...
        printf("\n");
#endif
        //Due to Continuum Packet's Crc8 checksum some invalid packets should be ignored.
        //Funny thing is on SubSpace Protocol there was never any invalid packets, other then packetloss.
        if(RecvLength > 0)
            ProcessCorePackets(connection, buffer, RecvLength, 0);
        return;
    }
    ...
    if ( packetAttachment->EncryptMode == 0 )
        *(unsigned int *)(&buffer[2]) = abs(*(unsigned int *)(&buffer[2]));
    else if ( packetAttachment->EncryptMode == 1 )
        *(unsigned int *)(&buffer[2]) =  -abs(*(unsigned int *)(&buffer[2]));
    ...
    if ( v16 != -1 && packetAttachment->NewConnectionRequestAddress )
    {
        ...
        if ( ((int (__cdecl *)(struct in_addr, unsigned short, struct CONNECTION *))packetAttachment->NewConnectionRequestAddress)(RemoteIP, RemotePort, &packetAttachment->FreshConnections[v16]) )
        {
            ...
            if ( ProtocolVersion == 0x1 ) { //SubSpace
                buf[0] = 0;
            ...
            } else if ( ProtocolVersion == 0x10 ) { // Continuum Step 1 Array from Scrty1 File
                //Older version Step2 Transformer is still not ported over, will fix later.
                packetAttachment->FreshConnections[v16].ServerKey = abs(*(unsigned int *)(&buffer[2]));
                packetAttachment->FreshConnections[v16].ContinuumKeyExpansionOffset = rand() % 1024;
                Step1ContinuumResponseBuffer[0] = 0;
                Step1ContinuumResponseBuffer[1] = 0x10; //0x10 - Continuum Encryption Response (Server)
                *(unsigned int *)&Step1ContinuumResponseBuffer[2] = packetAttachment->FreshConnections[v16].ServerKey;
                *(unsigned int *)&Step1ContinuumResponseBuffer[6] = Security.Scrty1File.Step1[packetAttachment->FreshConnections[v16].ContinuumKeyExpansionOffset].Key;
                SendPacket(&packetAttachment->FreshConnections[v16], Step1ContinuumResponseBuffer, 10);
            } else if( ProtocolVersion == 0x11 ) { // Continuum Step 1 & 2 Array from Scrty1 File
                //This is the newer version much easier since all you have to is run Step3 transformer, Step1and2 are done for you.
                //TODO: MIGHT HAVE TO MAKE IMPLEMENT THE RE-ENTER ZONE TRACKER
                //Because it doesn't allow you to enter a zone from a second attempt only a third attempt will work.
                //The ServerKey in Continuum subgame is split into 2 Byte sections.
                //First 2 bytes keep track how much times you re-entered this zone.
                //Second 2 bytes are a pseudo -random number generator.
                //Together they form a 4 byte ServerKey
                //Instead of going all fancy I just decided to use the ServerKey from SubSpace Protocol.

                packetAttachment->FreshConnections[v16].ServerKey = abs(*(unsigned int *)(&buffer[2]));
                packetAttachment->FreshConnections[v16].ContinuumKeyExpansionOffset = rand() % 1024;
                Step1And2ContinuumResponseBuffer[0] = 0;
                Step1And2ContinuumResponseBuffer[1] = 0x10; //0x10 - Continuum Encryption Response (Server)
                *(unsigned int *)&Step1And2ContinuumResponseBuffer[2] = packetAttachment->FreshConnections[v16].ServerKey;
                *(unsigned int *)&Step1And2ContinuumResponseBuffer[6] = Security.Scrty1File.Step1And2[packetAttachment->FreshConnections[v16].ContinuumKeyExpansionOffset].Key;
                *(unsigned short *)&Step1And2ContinuumResponseBuffer[10] = 1;
                SendPacket(&packetAttachment->FreshConnections[v16], Step1And2ContinuumResponseBuffer, 12);
            }
        } else {
...
void __cdecl SendPacket(struct CONNECTION *connection, unsigned char *buf, unsigned int len)
{
    ...
    //States
    // 1 - Start up (no encryption yet)
    // 2 - Encryption is ready for use.
    // 3 - Normal Operation.
    // 4 - Disconnected.
    if ( connection->state != 4 )
    {
        //ServerKey of 0 is never encrypted.
        //Packet Length greater then 2 is encrypted.
        if( connection->ServerKey >= 0 && len > 2 ) {
            memcpy(buffer, buf, len);
#if (DEBUG_SEND_PACKETS == 1)
            printf("[SEND][Size: %d]: ", len);
            int i = 0;
            for(i = 0; i < len; i++) {
                printf("%X ", buffer[i]);
            }
            printf("\n");
#endif
            //0x01 - Encryption Request  Core packet never encrypted on SubSpace
            //0x02 - Encryption Response Core packet never encrypted on SubSpace
            if(connection->ProtocolVersion == 1 && buffer[1] != 1 && buffer[1] != 2 ) { //SubSpace
                unsigned short StartOffset = 1;
                if ( !buf[0] ) //If the byte at offset 0 is 0x00
                    StartOffset = 2; //Skip Next Byte as its a Core PacketType
                ...
                for(unsigned short Counter = 0; Counter < len; Counter += 4) {
                    StartServerKey ^= *(unsigned int *)&buffer[Counter+StartOffset] ^ *(unsigned int *)&(connection->SSEncryptionTable[Counter]);
                    *(unsigned int *)&buffer[Counter+StartOffset] = StartServerKey;
                }
                //0x01 - Encryption Request  Core packet never encrypted on Continuum
                //0x10 - Continuum Encryption Response (Server) Core packet never encrypted on Continuum
            } else if( (connection->ProtocolVersion == 0x11 || connection->ProtocolVersion == 0x10)
                       && buffer[1] != 1 && buffer[1] != 0x10 ) { //Continuum Step1And2 Encrypt()
                ContinuumEncrypt(connection, buffer, &len);
            }
...
void __cdecl ProcessCorePackets(struct CONNECTION *connection, unsigned char *buffer, int PacketLength, int a4)
{
    ...
    unsigned char ContinuumMimicLoginPacket[6];
    ...
    if ( buffer[0] == 0 ) //Core Packets
    {
        ...
        case 2:	//0x02 - Encryption Response
            ...
            goto RETURN_FREE_MEMORY;
        case 0x11: //0x11 - Continuum Encryption Request (Client)
            v8 = *(unsigned int *)&buffer[2];	//Client Encryption Key
            if ( abs(connection->ServerKey) == abs(v8) ) {
                if ( connection->ProtocolVersion == 0x10 ) { // Continuum Step 1 Array from Scrty1 File
                    printf("Continuum 0x10 Protocol Triggered!\n");
                    //Older version can't really do this since Step2 transformer is missing (too lazy)
                    memcpy(
                        connection->ContinuumExpandedKey,
                        Security.Scrty1File.Step1[connection->ContinuumKeyExpansionOffset].ExpandedKey,
                        80);

                    //TODO: Add ContinuumEncryptionMD5Step2AndStep3();
                } else if( connection->ProtocolVersion == 0x11 ) { // Continuum Step 1 & 2 Array from Scrty1 File
                    printf("Continuum 0x11 Protocol Is used\n");
                    memcpy(
                        connection->ContinuumExpandedKey,
                        Security.Scrty1File.Step1And2[connection->ContinuumKeyExpansionOffset].ExpandedKey,
                        80);

                    ContinuumEncryptionMD5Step3(
                        connection->ContinuumExpandedKey,
                        connection->ServerKey);

                    connection->state = 2; //2 = Encryption is initialized
                    ContinuumMimicLoginPacket[0] = 0;
                    ContinuumMimicLoginPacket[1] = 2;	// 0x02 - Encryption Response
                    *(unsigned int *)&ContinuumMimicLoginPacket[2] = connection->ServerKey;
                    SendPacket(connection, ContinuumMimicLoginPacket, 6);
                }
            }
            goto RETURN_FREE_MEMORY;
        default:
...
unsigned int ROL(unsigned int var, unsigned int amount)
{
    amount &= 31;
...
void ContinuumEncryptionMD5Step3(unsigned int *ArrayOfIntegers, unsigned int Key)
{
    /*Took about a hour to step though this completely to make sure it has no errors.
        No signed integers here to make sure it all works exactly like on OllyDebugger
        100% perfect -fatrolls
    */
    //int ArrayOfIntegers; // eax@1
    ...
    unsigned int v44; // ST10_4@1
    unsigned int v45; // esi@1
    unsigned int v46; // esi@1
    unsigned int v47; // edi@1
    unsigned int v48; // edi@1
    unsigned int v49; // ebx@1
    unsigned int v50; // ebx@1
    unsigned int v51; // ecx@1
    unsigned int v52; // esi@1
    unsigned int v53; // ecx@1
    unsigned int v54; // esi@1
    unsigned int v55; // edi@1
    unsigned int v56; // edi@1
    ...
    unsigned int v63; // esi@1
    unsigned int v64; // edi@1
    unsigned int v65; // edi@1
    unsigned int v66; // ebx@1
    unsigned int v67; // ebx@1
    unsigned int v68; // ecx@1
    unsigned int v69; // ecx@1
    unsigned int v70; // esi@1
    unsigned int v71; // esi@1
    unsigned int v72; // edi@1
    unsigned int v73; // edi@1
    unsigned int v74; // ebx@1
    unsigned int v75; // ebx@1
    unsigned int v76; // ecx@1
    unsigned int v77; // ecx@1
    unsigned int v78; // esi@1
    unsigned int v79; // esi@1
    unsigned int v80; // edi@1
    unsigned int v81; // edi@1
    unsigned int v82; // edx@1
    unsigned int v83; // edx@1
    unsigned int v84; // ecx@1
    unsigned int v85; // ecx@1
    unsigned int v86; // esi@1
    unsigned int v87; // esi@1
    unsigned int v88; // edi@1
    unsigned int v89; // edi@1
    unsigned int v90; // edx@1
    unsigned int v91; // edx@1
    unsigned int v92; // ecx@1
    unsigned int v93; // ecx@1
    unsigned int v94; // esi@1
    unsigned int v95; // esi@1
    unsigned int v96; // ebp@1
    unsigned int v97; // edi@1
    unsigned int v98; // edi@1
    unsigned int v99; // edx@1
    unsigned int v100; // edx@1
    unsigned int v101; // ecx@1
    unsigned int v102; // ecx@1
    unsigned int v103; // esi@1
    unsigned int v104; // esi@1
    unsigned int v105; // edi@1
    unsigned int v106; // edi@1
    unsigned int v107; // edx@1
    unsigned int v108; // edx@1
    unsigned int v109; // ecx@1
    unsigned int v110; // ecx@1
    unsigned int v111; // esi@1
    unsigned int v112; // esi@1
    unsigned int v113; // edi@1
    unsigned int v114; // edi@1
    unsigned int v115; // edx@1
    unsigned int v116; // edx@1
    unsigned int v117; // ecx@1
    unsigned int v118; // ecx@1
    unsigned int v119; // esi@1
    unsigned int v120; // esi@1
    unsigned int v121; // edi@1
    unsigned int v122; // edi@1
    unsigned int v123; // ebx@1
    unsigned int v124; // ebx@1
    unsigned int v125; // edx@1
    unsigned int v126; // edx@1
    unsigned int v127; // esi@1
    unsigned int v128; // esi@1
    unsigned int v129; // ecx@1
    unsigned int v130; // ebp@1
    unsigned int v131; // edi@1
    unsigned int v132; // edi@1
    unsigned int v133; // edx@1
    unsigned int v134; // ecx@1
    unsigned int v135; // edx@1
    signed int v136; // ecx@1
    void *v137; // [sp+18h] [bp-18h]@1
    int v138[4]; // [sp+20h] [bp-10h]@1

    ArrayOfIntegers[0] ^= Key;
    v3 = ROL(ArrayOfIntegers[0] - 0x28A5AA39, 7);
    v3 -= 0x10325537;
    v4 = ROL((ArrayOfIntegers[1] + (v3 & 0xEFCDAAC9 | ~v3 & 0x98BACD4E)) - 0x665F354, 12);
    v5 = v3 + v4;
    v6 = ROL(ArrayOfIntegers[2] + (v3 & v5 | ~v5 & 0xEFCDAAC9) - 0x4324C047, 17);
    v7 = v5 + v6;
    v9 = ROL(ArrayOfIntegers[3] + (v7 & v5 | v3 & ~v7) - 0x4EA515D9, 22);
    v10 = v7 + v9;
    v11 = ROL(ArrayOfIntegers[4] + (v10 & v7 | v5 & ~v10) + v3 - 0xA8CF6E1, 7);
    v12 = v10 + v11;
    v13 = ROL(ArrayOfIntegers[5] + (v12 & v10 | v7 & ~v12) + v5 + 0x4787C79A, 12);
    v14 = v12 + v13;
    v15 = ROL(ArrayOfIntegers[6] + (v12 & v14 | v10 & ~v14) + v7 - 0x57C7B85D, 17);
    v16 = v14 + v15;
    v17 = ROL(ArrayOfIntegers[7] + (v16 & v14 | v12 & ~v16) + v10 - 0x2B96B4F, 22);
    v18 = v16 + v17;
    v19 = ROL(ArrayOfIntegers[8] + (v18 & v16 | v14 & ~v18) + v12 + 0x68809968, 7);
    v20 = v18 + v19;
    v21 = ROL(ArrayOfIntegers[9] + (v20 & v18 | v16 & ~v20) + v14 - 0x74BB09E1, 12);
    v22 = v20 + v21;
    v23 = ROL(ArrayOfIntegers[10] + (v20 & v22 | v18 & ~v22) + v16 - 0x70A5FF, 17);
    v24 = v22 + v23;
    v25 = ROL(ArrayOfIntegers[11] + (v24 & v22 | v20 & ~v24) + v18 - 0x767329F2, 22);
    v26 = v24 + v25;
    v27 = ROL(ArrayOfIntegers[12] + (v26 & v24 | v22 & ~v26) + v20 + 0x6B901992, 7);
    v28 = v26 + v27;
    v29 = ROL(ArrayOfIntegers[13] + (v28 & v26 | v24 & ~v28) + v22 - 0x2678FDD, 12);
    v30 = v28 + v29;
    v31 = ROL(ArrayOfIntegers[14] + (v28 & v30 | v26 & ~v30) + v24 - 0x5986BDC2, 17);
    v32 = v30 + v31;
    v33 = ROL(ArrayOfIntegers[15] + (v32 & v30 | v28 & ~v32) + v26 + 0x49740991, 22);
    v34 = v32 + v33;
    v35 = ROL(ArrayOfIntegers[1] + (v34 & v30 | v32 & ~v30) + v28 - 0x9EEDB2E, 5);
    v36 = v34 + v35;
    v37 = ROL(ArrayOfIntegers[6] + (v36 & v32 | v34 & ~v32) + v30 - 0x3ABF4D10, 9);
    v38 = v36 + v37;
    v39 = ROL(ArrayOfIntegers[11] + (v34 & v38 | v36 & ~v34) + v32 + 0x265E4BE1, 14);
    v40 = v38 + v39;
    v41 = ROL(ArrayOfIntegers[0] + (v36 & v40 | v38 & ~v36) + v34 - 0x164B39E6, 20);
    v42 = v40 + v41;
    v43 = ROL(ArrayOfIntegers[5] + (v42 & v38 | v40 & ~v38) + v36 - 0x29D8EE13, 5);
    v44 = v42 + v43;
    v45 = ROL(ArrayOfIntegers[10] + ((v42 + v43) & v40 | v42 & ~v40) + v38 + 0x25415E3, 9);
    v46 = v42 + v43 + v45;
    v47 = ROL(ArrayOfIntegers[15] + (v42 & v46 | (v42 + v43) & ~v42) + v40 - 0x275EA8CF, 14);
    v48 = v46 + v47;
    v49 = ROL(ArrayOfIntegers[4] + ((v42 + v43) & v48 | v46 & ~(v42 + v43)) + v42 - 0x182C0588, 20);
    v50 = v48 + v49;
    v51 = ROL(ArrayOfIntegers[9] + (v50 & v46 | v48 & ~v46) + v44 + 0x21E4CC56, 5);
    v53 = v50 + v51;
    v52 = ROL(ArrayOfIntegers[14] + ((v50 + v51) & v48 | v50 & ~v48) + v46 - 0x3CC8F99A, 9);
    v54 = v53 + v52;
    v55 = ROL(ArrayOfIntegers[3] + (v50 & v54 | v53 & ~v50) + v48 - 0xB2AFDC9, 14);
    v56 = v54 + v55;
    v58 = ROL(ArrayOfIntegers[8] + (v53 & v56 | v54 & ~v53) + v50 + 0x455A155D, 20);
    v59 = v56 + v58;
    v60 = ROL(ArrayOfIntegers[13] + (v59 & v54 | v56 & ~v54) + v53 - 0x56CC174B, 5);
    v62 = v59 + v60;
    v61 = ROL(ArrayOfIntegers[2] + ((v59 + v60) & v56 | v59 & ~v56) + v54 - 0x3105DB8, 9);
    v63 = v62 + v61;
    v64 = ROL(ArrayOfIntegers[7] + (v59 & v63 | v62 & ~v59) + v56 + 0x676F0069, 14);
    v65 = v63 + v64;
    v66 = ROL(ArrayOfIntegers[12] + (v62 & v65 | v63 & ~v62) + v59 - 0x72D6B2C6, 20);
    v67 = v65 + v66;
    v68 = ROL(ArrayOfIntegers[5] + (v67 ^ v65 ^ v63) + v62 - 0xE05C70E, 4);
    v69 = v67 + v68;
    v70 = ROL(ArrayOfIntegers[8] + (v69 ^ v67 ^ v65) + v63 - 0x788E08CF, 11);
    v71 = v69 + v70;
    v72 = ROL(ArrayOfIntegers[11] + (v69 ^ v67 ^ v71) + v65 + 0x6D3D6092, 16);
    v73 = v71 + v72;
    v74 = ROL(ArrayOfIntegers[14] + (v69 ^ v73 ^ v71) + v67 - 0x21AC644, 23);
    v75 = v73 + v74;
    v76 = ROL(ArrayOfIntegers[1] + (v75 ^ v73 ^ v71) + v69 - 0x5B41940C, 4);
    v77 = v75 + v76;
    v78 = ROL(ArrayOfIntegers[4] + (v77 ^ v75 ^ v73) + v71 + 0x4BDEC219, 11);
    v79 = v77 + v78;
    v80 = ROL(ArrayOfIntegers[7] + (v77 ^ v75 ^ v79) + v73 - 0x947B530, 16);
    v81 = v79 + v80;
    v82 = ROL(ArrayOfIntegers[10] + (v77 ^ v81 ^ v79) + v75 - 0x41404240, 23);
    v83 = v81 + v82;
    v84 = ROL(ArrayOfIntegers[13] + (v83 ^ v81 ^ v79) + v77 + 0x28937F76, 4);
    v85 = v83 + v84;
    v86 = ROL(ArrayOfIntegers[0] + (v85 ^ v83 ^ v81) + v79 - 0x155ED9B6, 11);
    v87 = v85 + v86;
    v88 = ROL(ArrayOfIntegers[3] + (v85 ^ v83 ^ v87) + v81 - 0x2B107ECB, 16);
    v89 = v87 + v88;
    v90 = ROL(ArrayOfIntegers[6] + (v85 ^ v89 ^ v87) + v83 + 0x4881CB5, 23);
    v91 = v89 + v90;
    v92 = ROL(ArrayOfIntegers[9] + (v91 ^ v89 ^ v87) + v85 - 0x262B2D77, 4);
    v93 = v91 + v92;
    v94 = ROL(ArrayOfIntegers[12] + (v93 ^ v91 ^ v89) + v87 - 0x199467AB, 11);
    v95 = v93 + v94;
    v97 = ROL(ArrayOfIntegers[15] + (v93 ^ v91 ^ v95) + v89 + 0x1FA27D48, 16);
    v98 = v95 + v97;
    v99 = ROL(ArrayOfIntegers[2] + (v93 ^ v98 ^ v95) + v91 - 0x3B53C82B, 23);
    v100 = v98 + v99;
    v101 = ROL(ArrayOfIntegers[0] + (v98 ^ (v100 | ~v95)) + v93 - 0xBCBDC0C, 6);
    v102 = v100 + v101;
    v103 = ROL(ArrayOfIntegers[7] + (v100 ^ (v102 | ~v98)) + v95 + 0x432A6E27, 10);
    v104 = v102 + v103;
    v105 = ROL(ArrayOfIntegers[14] + (v102 ^ (v104 | ~v100)) + v98 - 0x546BDDE9, 15);
    v106 = v104 + v105;
    v107 = ROL(ArrayOfIntegers[5] + (v104 ^ (v106 | ~v102)) + v100 - 0x36A5E77, 21);
    v108 = v106 + v107;
    v109 = ROL(ArrayOfIntegers[12] + (v106 ^ (v108 | ~v104)) + v102 + 0x65555873, 6);
    v110 = v108 + v109;
    v111 = ROL(ArrayOfIntegers[3] + (v108 ^ (v110 | ~v106)) + v104 - 0x70F332DE, 10);
    v112 = v110 + v111;
    v113 = ROL(ArrayOfIntegers[10] + (v110 ^ (v112 | ~v108)) + v106 - 0xA100A33, 15);
    v114 = v112 + v113;
    v115 = ROL(ArrayOfIntegers[1] + (v112 ^ (v114 | ~v110)) + v108 - 0x7A7BA39F, 21);
    v116 = v114 + v115;
    v117 = ROL(ArrayOfIntegers[8] + (v114 ^ (v116 | ~v112)) + v110 + 0x6FA57FFF, 6);
    v118 = v116 + v117;
    v119 = ROL(ArrayOfIntegers[15] + (v116 ^ (v118 | ~v114)) + v112 - 0xAD318B0, 10);
    v120 = v118 + v119;
    v121 = ROL(ArrayOfIntegers[6] + (v118 ^ (v120 | ~v116)) + v114 - 0x5CFEBB5C, 15);
    v122 = v120 + v121;
    v123 = ROL(ArrayOfIntegers[13] + (v120 ^ (v122 | ~v118)) + v116 + 0x4E085011, 21);
    v124 = v122 + v123;
    v125 = ROL(ArrayOfIntegers[4] + (v122 ^ (v124 | ~v120)) + v118 - 0x8AA80CE, 6);
    v126 = v124 + v125;
    v127 = ROL(ArrayOfIntegers[11] + (v124 ^ (v126 | ~v122)) + v120 + 0x453AF385, 10);
    v128 = v126 + v127;
    v131 = ROL(ArrayOfIntegers[2] + (v126 ^ (v128 | ~v124)) + v122 + 0x2AD7830B, 15);
    v132 = v128 + v131;
    v133 = ROL(ArrayOfIntegers[9] + (v128 ^ (v132 | ~v126)) + v124 - 0x14A92DDF, 21);
    v138[0] = v126 + 0x673522B1;
    v138[1] = v133 + v132 - 0x10325537;
    v138[2] = v132 - 0x674532B2;
    v138[3] = v128 + 0x113255C6;

    v135 = Key;
    v136 = 0;
    do
    {
        ArrayOfIntegers[v136] ^= v135 + v138[v136 & 3];
        v135 = ArrayOfIntegers[v136];
        v136++;
    }
    while ( v136 < 20 );
}
...
void ContinuumEncrypt(struct CONNECTION *connection, unsigned char *buffer, unsigned int *packetlen)
{
    //Appends a CRC Byte before encrypting the packet
    unsigned char CrcBuffer[*packetlen+1];
    memcpy(&CrcBuffer[1], buffer, *packetlen);
    CrcBuffer[0] = Crc8(&CrcBuffer[1], *packetlen);
    *packetlen += 1; //Make room for CRC Byte in front.
    memcpy(buffer, CrcBuffer, *packetlen);

    unsigned int sizeInBlocks = *packetlen / 8;
    unsigned int bytesRemaining = *packetlen;

    unsigned int lastUpperhalf = 0;
    unsigned int lastLowerhalf = 0;

    //Transform the Byte Array into a Integer Array so it's easier to understand
    unsigned int * processBuffer = (unsigned int *)buffer;

    for(unsigned int i = 0; i < sizeInBlocks; ++i)
    {
        unsigned int lowerhalf = processBuffer[0] ^ lastLowerhalf;
        unsigned int upperhalf = processBuffer[1] ^ lastUpperhalf;

        for(unsigned int j = 0; j < 20; ++j)
        {
            lowerhalf += connection->ContinuumExpandedKey[j++];
            lowerhalf = ROL(lowerhalf, upperhalf & 0xFF);

            upperhalf += connection->ContinuumExpandedKey[j];
            upperhalf = ROL(upperhalf, lowerhalf & 0xFF);
        }

        lastLowerhalf = lowerhalf;
        lastUpperhalf = upperhalf;
        processBuffer[0] = lowerhalf;
        processBuffer[1] = upperhalf;
        processBuffer += 2;
        bytesRemaining -= 8;
    }
    ...
    if(bytesRemaining >= 4)
    {
        unsigned int block = processBuffer[0] ^ lastLowerhalf;

        for(unsigned int i = 0; i < 20; ++i)
        {
            block += connection->ContinuumExpandedKey[i++];
            block = ROL(block, lastUpperhalf & 0xFF);

            lastUpperhalf += connection->ContinuumExpandedKey[i];
        }

        lastLowerhalf = block;
        processBuffer[0] = block;
        ++processBuffer;
        bytesRemaining -= 4;
    }

    unsigned char * processBufferBytes = (unsigned char *)processBuffer;
    for(unsigned int i = 0; i < bytesRemaining; ++i)
    {
        unsigned char currentByte = processBufferBytes[i] ^ lastLowerhalf;

        for(unsigned int j = 0; j < 20; ++j)
        {
            currentByte += connection->ContinuumExpandedKey[j++] & 0xFF;
            currentByte += lastUpperhalf & 0xFF;

            lastUpperhalf += connection->ContinuumExpandedKey[j];
        }

        lastLowerhalf = currentByte;
        processBufferBytes[i] = currentByte;
    }
}
...
void ContinuumDecrypt(struct CONNECTION *connection, unsigned char *buffer, unsigned int *packetlen)
{
    ...
    for(unsigned int i = 0; i < sizeInBlocks; ++i)
    {
        unsigned int lowerhalf = processBuffer[0];
        unsigned int upperhalf = processBuffer[1];
        ...
        lastLowerhalf = processBuffer[0];
        lastUpperhalf = processBuffer[1];
        processBuffer[0] = lowerhalf;
        processBuffer[1] = upperhalf;
        processBuffer += 2;
        bytesRemaining -= 8;
    ...
    if(bytesRemaining >= 4)
    {
        unsigned int roramount = lastUpperhalf;
        unsigned int block = processBuffer[0];

        for(unsigned int i = 1; i < 20; i += 2)
            roramount += connection->ContinuumExpandedKey[i];
        ...
        for(int i = 19; i > 0; i -= 2)
        {
            ...
            block -= connection->ContinuumExpandedKey[i - 1];
        }
    ...
    //Transform the Integer Array back into a Byte Array
    unsigned char * processBufferBytes = (unsigned char *)processBuffer;

    for(unsigned int i = 0; i < bytesRemaining; ++i)
    {
        unsigned char currentByte = processBufferBytes[i];
        unsigned int accum = lastUpperhalf;

        for(unsigned int j = 1; j < 20; j += 2)
            accum += connection->ContinuumExpandedKey[j];
        ...
        {
            accum -= connection->ContinuumExpandedKey[j];
            currentByte -= ((connection->ContinuumExpandedKey[j - 1] & 0xFF) + (accum & 0xFF));
        }

        currentByte ^= (lastLowerhalf & 0xFF);
        lastLowerhalf = processBufferBytes[i];
        processBufferBytes[i] = currentByte;
    }

    if(Crc8(&buffer[1], *packetlen - 1) == buffer[0]) {
        //Remove Crc8 from packet so it will be compatiable with Subspace Protocol.
        *packetlen -= 1; //Size of packet is smaller by 1
        memcpy(buffer, &buffer[1], *packetlen);
    } else {
        *packetlen = 0;
        printf("Crc8 error while decrypting Continuum Packet, packet discarded\n");
    }
}
...
unsigned char Crc8(unsigned char *packet, int length)
{
    ...
    for(int i = 0; i < length; ++i)
        crc = CRC8LookupTable[crc ^ packet[i]];

    return crc;
}

/*
void * ContinuumEncryptionMD5Step1(void **ArrayOfIntegers, int a2, int a3)
{
  void **v3; // esi@1
  int aa2; // eax@1
  void *v5; // eax@1
  ...
  int v267; // ecx@3
  int v268; // edx@3
  signed int v269; // ecx@3
  void **v270; // [sp+14h] [bp-1Ch]@1
  int v271[4]; // [sp+20h] [bp-10h]@1
  ...
  aa2 = a2 << 16;
  LOWORD(aa2) = a2;
  memset32(*ArrayOfIntegers, aa2, 20); //20 DWORD's so thats 20x4 = 80 bytes
  *(unsigned int *)*v3 ^= a3;
  v6 = ROL(ArrayOfIntegers[0] - 0x28A5AA39, 7);
  v6 -= 0x10325537;
  v7 = ROL(ArrayOfIntegers[1] + (v6 & 0xEFCDAAC9 | ~v6 & 0x98BACD4E) - 0x665F354, 12);
  v8 = v6 + v7;
  v9 = ROL(ArrayOfIntegers[2] + (v6 & v8 | ~v8 & 0xEFCDAAC9) - 0x4324C047, 17);
  v10 = v8 + v9;
  v11 = ROL(ArrayOfIntegers[3] + (v10 & v8 | v6 & ~v10) - 0x4EA515D9, 22);
  v12 = v10 + v11;
  v13 = ROL(ArrayOfIntegers[4] + (v12 & v10 | v8 & ~v12) + v6 - 0xA8CF6E1, 7);
  v14 = v12 + v13;
  v15 = ROL(ArrayOfIntegers[5] + (v14 & v12 | v10 & ~v14) + v8 + 0x4787C79A, 12);
  v16 = v14 + v15;
  v17 = ROL(ArrayOfIntegers[6] + (v14 & v16 | v12 & ~v16) + v10 - 0x57C7B85D, 17);
  v18 = v16 + v17;
  v19 = ROL(ArrayOfIntegers[7] + (v18 & v16 | v14 & ~v18) + v12 - 0x2B96B4F, 22);
  v20 = v18 + v19;
  v21 = ROL(ArrayOfIntegers[8] + (v20 & v18 | v16 & ~v20) + v14 + 0x68809968, 7);
  v22 = v20 + v21;
  v23 = ROL(ArrayOfIntegers[9] + (v22 & v20 | v18 & ~v22) + v16 - 0x74BB09E1, 12);
  v24 = v22 + v23;
  v25 = ROL(ArrayOfIntegers[10] + (v22 & v24 | v20 & ~v24) + v18 - 0x70A5FF, 17);
  v26 = v24 + v25;
  v27 = ROL(ArrayOfIntegers[11] + (v26 & v24 | v22 & ~v26) + v20 - 0x767329F2, 22);
  v28 = v26 + v27;
  v29 = ROL(ArrayOfIntegers[12] + (v28 & v26 | v24 & ~v28) + v22 + 0x6B901992, 7);
  v30 = v28 + v29;
  v31 = ROL(ArrayOfIntegers[13] + (v30 & v28 | v26 & ~v30) + v24 - 0x2678FDD, 12);
  v32 = v30 + v31;
  v33 = ROL(ArrayOfIntegers[14] + (v30 & v32 | v28 & ~v32) + v26 - 0x5986BDC2, 17);
  v34 = v32 + v33;
  v35 = ROL(ArrayOfIntegers[15] + (v34 & v32 | v30 & ~v34) + v28 + 0x49740991, 22);
  v36 = v34 + v35;
  v37 = ROL(ArrayOfIntegers[1] + (v36 & v32 | v34 & ~v32) + v30 - 0x9EEDB2E, 5);
  v38 = v36 + v37;
  v39 = ROL(ArrayOfIntegers[6] + (v38 & v34 | v36 & ~v34) + v32 - 0x3ABF4D10, 9);
  v40 = v38 + v39;
  v41 = ROL(ArrayOfIntegers[11] + (v36 & v40 | v38 & ~v36) + v34 + 0x265E4BE1, 14);
  v42 = v40 + v41;
  v43 = ROL(ArrayOfIntegers[0] + (v38 & v42 | v40 & ~v38) + v36 - 0x164B39E6, 20);
  v44 = v42 + v43;
  v45 = ROL(ArrayOfIntegers[5] + (v44 & v40 | v42 & ~v40) + v38 - 0x29D8EE13, 5);
  v47 = v44 + v45;
  v46 = ROL(ArrayOfIntegers[10] + ((v44 + v45) & v42 | v44 & ~v42) + v40 + 0x25415E3, 9);
  v48 = v47 + v46;
  v49 = ROL(ArrayOfIntegers[15] + (v44 & v48 | v47 & ~v44) + v42 - 0x275EA8CF, 14);
  v50 = v48 + v49;
  v51 = ROL(ArrayOfIntegers[4] + (v47 & v50 | v48 & ~v47) + v44 - 0x182C0588, 20);
  v52 = v50 + v51;
  v53 = ROL(ArrayOfIntegers[9] + (v52 & v48 | v50 & ~v48) + v47 + 0x21E4CC56, 5);
  v55 = v52 + v53;
  v54 = ROL(ArrayOfIntegers[14] + ((v52 + v53) & v50 | v52 & ~v50) + v48 - 0x3CC8F99A, 9);
  v56 = v55 + v54;
  v57 = ROL(ArrayOfIntegers[3] + (v52 & v56 | v55 & ~v52) + v50 - 0xB2AFDC9, 14);
  v58 = v56 + v57;
  v60 = ROL(ArrayOfIntegers[8] + (v55 & v58 | v56 & ~v55) + v52 + 0x455A155D, 20);
  v61 = v58 + v60;
  v62 = ROL(ArrayOfIntegers[13] + (v61 & v56 | v58 & ~v56) + v55 - 0x56CC174B, 5);
  v64 = v61 + v62;
  v63 = ROL(ArrayOfIntegers[2] + ((v61 + v62) & v58 | v61 & ~v58) + v56 - 0x3105DB8, 9);
  v65 = v64 + v63;
  v66 = ROL(ArrayOfIntegers[7] + (v61 & v65 | v64 & ~v61) + v58 + 0x676F0069, 14);
  v67 = v65 + v66;
  v68 = ROL(ArrayOfIntegers[12] + (v64 & v67 | v65 & ~v64) + v61 - 0x72D6B2C6, 20);
  v69 = v67 + v68;
  v70 = ROL(ArrayOfIntegers[5] + (v69 ^ v67 ^ v65) + v64 - 0xE05C70E, 4);
  v71 = v69 + v70;
  v72 = ROL(ArrayOfIntegers[8] + (v71 ^ v69 ^ v67) + v65 - 0x788E08CF, 11);
  v73 = v71 + v72;
  v74 = ROL(ArrayOfIntegers[11] + (v71 ^ v69 ^ v73) + v67 + 0x6D3D6092, 16);
  v75 = v73 + v74;
  v76 = ROL(ArrayOfIntegers[14] + (v71 ^ v75 ^ v73) + v69 - 0x21AC644, 23);
  v77 = v75 + v76;
  v78 = ROL(ArrayOfIntegers[1] + (v77 ^ v75 ^ v73) + v71 - 0x5B41940C, 4);
  v79 = v77 + v78;
  v80 = ROL(ArrayOfIntegers[4] + (v79 ^ v77 ^ v75) + v73 + 0x4BDEC219, 11);
  v81 = v79 + v80;
  v82 = ROL(ArrayOfIntegers[7] + (v79 ^ v77 ^ v81) + v75 - 0x947B530, 16);
  v83 = v81 + v82;
  v84 = ROL(ArrayOfIntegers[10] + (v79 ^ v83 ^ v81) + v77 - 0x41404240, 23);
  v85 = v83 + v84;
  v86 = ROL(ArrayOfIntegers[13] + (v85 ^ v83 ^ v81) + v79+ 0x28937F76, 4);
  v87 = v85 + v86;
  v88 = ROL(ArrayOfIntegers[0] + (v87 ^ v85 ^ v83) + v81 - 0x155ED9B6, 11);
  v89 = v87 + v88;
  v90 = ROL(ArrayOfIntegers[3] + (v87 ^ v85 ^ v89) + v83 - 0x2B107ECB, 16);
  v91 = v89 + v90;
  v92 = ROL(ArrayOfIntegers[6] + (v87 ^ v91 ^ v89) + v85 + 0x4881CB5, 23);
  v93 = v91 + v92;
  v94 = ROL(ArrayOfIntegers[9] + (v93 ^ v91 ^ v89) + v87 - 0x262B2D77, 4);
  v95 = v93 + v94;
  v96 = ROL(ArrayOfIntegers[12] + (v95 ^ v93 ^ v91) + v89 - 0x199467AB, 11);
  v97 = v95 + v96;
  v98 = ROL(ArrayOfIntegers[15] + (v95 ^ v93 ^ v97) + v91 + 0x1FA27D48, 16);
  v100 = v97 + v98;
  v101 = ROL(ArrayOfIntegers[2] + (v95 ^ v100 ^ v97) + v93 - 0x3B53C82B, 23);
  v102 = v100 + v101;
  v103 = ROL(ArrayOfIntegers[0] + (v100 ^ (v102 | ~v97)) + v95 - 0xBCBDC0C, 6);
  v104 = v102 + v103;
  v105 = ROL(ArrayOfIntegers[7] + (v102 ^ (v104 | ~v100)) + v97 + 0x432A6E27, 10);
  v106 = v104 + v105;
  v107 = ROL(ArrayOfIntegers[14] + (v104 ^ (v106 | ~v102)) + v100 - 0x546BDDE9, 15);
  v108 = v106 + v107;
  v109 = ROL(ArrayOfIntegers[5] + (v106 ^ (v108 | ~v104)) + v102 - 0x36A5E77, 21);
  v110 = v108 + v109;
  v111 = ROL(ArrayOfIntegers[12] + (v108 ^ (v110 | ~v106)) + v104 + 0x65555873, 6);
  v112 = v110 + v111;
  v113 = ROL(ArrayOfIntegers[3] + (v110 ^ (v112 | ~v108)) + v106 - 0x70F332DE, 10);
  v114 = v112 + v113;
  v115 = ROL(ArrayOfIntegers[10] + (v112 ^ (v114 | ~v110)) + v108 - 0xA100A33, 15);
  v116 = v114 + v115;
  v117 = ROL(ArrayOfIntegers[1] + (v114 ^ (v116 | ~v112)) + v110 - 0x7A7BA39F, 21);
  v118 = v116 + v117;
  v119 = ROL(ArrayOfIntegers[8] + (v116 ^ (v118 | ~v114)) + v112 + 0x6FA57FFF, 6);
  v120 = v118 + v119;
  v121 = ROL(ArrayOfIntegers[15] + (v118 ^ (v120 | ~v116)) + v114 - 0xAD318B0, 10);
  v122 = v120 + v121;
  v123 = ROL(ArrayOfIntegers[6] + (v120 ^ (v122 | ~v118)) + v116 - 0x5CFEBB5C, 15);
  v124 = v122 + v123;
  v125 = ROL(ArrayOfIntegers[13] + (v122 ^ (v124 | ~v120)) + v118 + 0x4E085011, 21);
  v126 = v124 + v125;
  v127 = ROL(ArrayOfIntegers[4] + (v124 ^ (v126 | ~v122)) + v120 - 0x8AA80CE, 6);
  v128 = v126 + v127;
  v129 = ROL(ArrayOfIntegers[11] + (v126 ^ (v128 | ~v124)) + v122 + 0x453AF385, 10);
  v130 = v128 + v129;
  v133 = ROL(ArrayOfIntegers[2] + (v128 ^ (v130 | ~v126)) + v124 + 0x2AD7830B, 15);
  v134 = v130 + v133;
  v135 = ROL(ArrayOfIntegers[9] + (v130 ^ (v134 | ~v128)) + v126 - 0x14A92DDF, 21);
  v271[0] = v128 + 0x673522B1;
  v271[1] = v135 + v134 - 0x10325537;
  v271[2] = v134 - 0x674532B2;
  v271[3] = v130 + 0x113255C6;

  v136 = a3;
  v137 = 0;
  do
  {
    ArrayOfIntegers[v137] ^= v136 + v271[v137 & 3];
    v136 = ArrayOfIntegers[v137++];
  }
  while ( v137 < 20 );

  ArrayOfIntegers[0] ^= a3;
  v139 = ROL(ArrayOfIntegers[0] - 0x28A5AA39, 7);
  v139 -= 0x10325537u;
  v140 = ROL(ArrayOfIntegers[1] + (v139 & 0xEFCDAAC9 | ~v139 & 0x98BACD4E) - 0x665F354, 12);
  v141 = v139 + v140;
  v142 = ROL( ArrayOfIntegers[2] + (v139 & v141 | ~v141 & 0xEFCDAAC9) - 0x4324C047, 17);
  v143 = v141 + v142;
  v144 = ROL(ArrayOfIntegers[3] + (v143 & v141 | v139 & ~v143) - 0x4EA515D9, 22);
  v145 = v143 + v144;
  v146 = ROL(ArrayOfIntegers[4] + (v145 & v143 | v141 & ~v145) + v139 - 0xA8CF6E1, 7);
  v147 = v145 + v146;
  v148 = ROL(ArrayOfIntegers[5] + (v147 & v145 | v143 & ~v147) + v141 + 0x4787C79A, 12);
  v149 = v147 + v148;
  v150 = ROL(ArrayOfIntegers[6] + (v147 & v149 | v145 & ~v149) + v143 - 0x57C7B85D, 17);
  v151 = v149 + v150;
  v152 = ROL(ArrayOfIntegers[7] + (v151 & v149 | v147 & ~v151) + v145 - 0x2B96B4F, 22);
  v153 = v151 + v152;
  v154 = ROL(ArrayOfIntegers[8] + (v153 & v151 | v149 & ~v153) + v147 + 0x68809968, 7);
  v155 = v153 + v154;
  v156 = ROL(ArrayOfIntegers[9] + (v155 & v153 | v151 & ~v155) + v149 - 0x74BB09E1, 12);
  v157 = v155 + v156;
  v158 = ROL(ArrayOfIntegers[10] + (v155 & v157 | v153 & ~v157) + v151 - 0x70A5FF, 17);
  v159 = v157 + v158;
  v160 = ROL(ArrayOfIntegers[11] + (v159 & v157 | v155 & ~v159) + v153 - 0x767329F2, 22);
  v161 = v159 + v160;
  v162 = ROL(ArrayOfIntegers[12] + (v161 & v159 | v157 & ~v161) + v155 + 0x6B901992, 7);
  v163 = v161 + v162;
  v164 = ROL(ArrayOfIntegers[13] + (v163 & v161 | v159 & ~v163) + v157 - 0x2678FDD, 12);
  v165 = v163 + v164;
  v166 = ROL(ArrayOfIntegers[14] + (v163 & v165 | v161 & ~v165) + v159 - 0x5986BDC2, 17);
  v167 = v165 + v166;
  v168 = ROL(ArrayOfIntegers[15] + (v167 & v165 | v163 & ~v167) + v161 + 0x49740991, 22);
  v169 = v167 + v168;
  v170 = ROL(ArrayOfIntegers[1] + (v169 & v165 | v167 & ~v165) + v163 - 0x9EEDB2E, 5);
  v171 = v169 + v170;
  v172 = ROL(ArrayOfIntegers[6] + (v171 & v167 | v169 & ~v167) + v165 - 0x3ABF4D10, 9);
  v173 = v171 + v172;
  v174 = ROL(ArrayOfIntegers[11] + (v169 & v173 | v171 & ~v169) + v167 + 0x265E4BE1, 14);
  v175 = v173 + v174;
  v176 = ROL(ArrayOfIntegers[0] + (v171 & v175 | v173 & ~v171) + v169 - 0x164B39E6, 20);
  v177 = v175 + v176;
  v178 = ROL(ArrayOfIntegers[5] + (v177 & v173 | v175 & ~v173) + v171 - 0x29D8EE13, 5);
  v180 = v177 + v178;
  v179 = ROL(ArrayOfIntegers[10] + ((v177 + v178) & v175 | v177 & ~v175) + v173 + 0x25415E3, 9);
  v181 = v180 + v179;
  v182 = ROL(ArrayOfIntegers[15] + (v177 & v181 | v180 & ~v177) + v175 - 0x275EA8CF, 14);
  v183 = v181 + v182;
  v184 = ROL(ArrayOfIntegers[4] + (v180 & v183 | v181 & ~v180) + v177 - 0x182C0588, 20);
  v185 = v183 + v184;
  v186 = ROL(ArrayOfIntegers[9] + (v185 & v181 | v183 & ~v181) + v180 + 0x21E4CC56, 5);
  v187 = v185 + v186;
  v188 = ROL(ArrayOfIntegers[14] + ((v185 + v186) & v183 | v185 & ~v183) + v181 - 0x3CC8F99A, 9);
  v189 = v185 + v186 + v188;
  v190 = ROL(ArrayOfIntegers[3] + v183 + (v185 & v189 | (v185 + v186) & ~v185) - 0xB2AFDC9, 14);
  v191 = v189 + v190;
  v193 = ROL(ArrayOfIntegers[8] + (v187 & v191 | v189 & ~v187) + v185 + 0x455A155D, 20);
  v194 = v191 + v193;
  v195 = ROL(ArrayOfIntegers[13] + (v194 & v189 | v191 & ~v189) + v187 - 0x56CC174B, 5);
  v197 = v194 + v195;
  v196 = ROL(ArrayOfIntegers[2] + ((v194 + v195) & v191 | v194 & ~v191) + v189 - 0x3105DB8, 9);
  v198 = v197 + v196;
  v199 = ROL(ArrayOfIntegers[7] + (v194 & v198 | v197 & ~v194) + v191 + 0x676F0069, 14);
  v200 = v198 + v199;
  v201 = ROL(ArrayOfIntegers[12] + (v197 & v200 | v198 & ~v197) + v194 - 0x72D6B2C6, 20);
  v202 = v200 + v201;
  v203 = ROL(ArrayOfIntegers[5] + (v202 ^ v200 ^ v198) + v197 - 0xE05C70E, 4);
  v204 = v202 + v203;
  v205 = ROL(ArrayOfIntegers[8] + (v204 ^ v202 ^ v200) + v198 - 0x788E08CF, 11);
  v206 = v204 + v205;
  v207 = ROL(ArrayOfIntegers[11] + (v204 ^ v202 ^ v206) + v200 + 0x6D3D6092, 16);
  v208 = v206 + v207;
  v209 = ROL(ArrayOfIntegers[14] + (v204 ^ v208 ^ v206) + v202 - 0x21AC644, 23);
  v210 = v208 + v209;
  v211 = ROL(ArrayOfIntegers[1] + (v210 ^ v208 ^ v206) + v204 - 0x5B41940C, 4);
  v212 = v210 + v211;
  v213 = ROL(ArrayOfIntegers[4] + (v212 ^ v210 ^ v208) + v206 + 0x4BDEC219, 11);
  v214 = v212 + v213;
  v215 = ROL(ArrayOfIntegers[7] + (v212 ^ v210 ^ v214) + v208 - 0x947B530, 16);
  v216 = v214 + v215;
  v217 = ROL(ArrayOfIntegers[10] + (v212 ^ v216 ^ v214) + v210 - 0x41404240, 23);
  v218 = v216 + v217;
  v219 = ROL(ArrayOfIntegers[13] + (v218 ^ v216 ^ v214) + v212 + 0x28937F76, 4);
  v220 = v218 + v219;
  v221 = ROL(ArrayOfIntegers[0] + (v220 ^ v218 ^ v216) + v214 - 0x155ED9B6, 11);
  v222 = v220 + v221;
  v223 = ROL(ArrayOfIntegers[3] + (v220 ^ v218 ^ v222) + v216 - 0x2B107ECB, 16);
  v224 = v222 + v223;
  v225 = ROL(ArrayOfIntegers[6] + (v220 ^ v224 ^ v222) + v218 + 0x4881CB5, 23);
  v226 = v224 + v225;
  v227 = ROL(ArrayOfIntegers[9] + (v226 ^ v224 ^ v222) + v220 - 0x262B2D77, 4);
  v228 = v226 + v227;
  v229 = ROL(ArrayOfIntegers[12] + (v228 ^ v226 ^ v224) + v222 - 0x199467AB, 11);
  v230 = v228 + v229;
  v232 = ROL(ArrayOfIntegers[15] + (v228 ^ v226 ^ v230) + v224 + 0x1FA27D48, 16);
  v233 = v230 + v232;
  v234 = ROL(ArrayOfIntegers[2] + (v228 ^ v233 ^ v230) + v226 - 0x3B53C82B, 23);
  v235 = v233 + v234;
  v236 = ROL(ArrayOfIntegers[0] + (v233 ^ (v235 | ~v230)) + v228 - 0xBCBDC0C, 6);
  v237 = v235 + v236;
  v238 = ROL(ArrayOfIntegers[7] + (v235 ^ (v237 | ~v233)) + v230 + 0x432A6E27, 10);
  v239 = v237 + v238;
  v240 = ROL(ArrayOfIntegers[14] + (v237 ^ (v239 | ~v235)) + v233 - 0x546BDDE9, 15);
  v241 = v239 + v240;
  v242 = ROL(ArrayOfIntegers[5] + (v239 ^ (v241 | ~v237)) + v235 - 0x36A5E77, 21);
  v243 = v241 + v242;
  v244 = ROL(ArrayOfIntegers[12] + (v241 ^ (v243 | ~v239)) + v237 + 0x65555873, 6);
  v245 = v243 + v244;
  v246 = ROL(ArrayOfIntegers[3] + (v243 ^ (v245 | ~v241)) + v239 - 0x70F332DE, 10);
  v247 = v245 + v246;
  v248 = ROL(ArrayOfIntegers[10] + (v245 ^ (v247 | ~v243)) + v241 - 0xA100A33, 15);
  v249 = v247 + v248;
  v250 = ROL(ArrayOfIntegers[1] + (v247 ^ (v249 | ~v245)) + v243 - 0x7A7BA39F, 21);
  v251 = v249 + v250;
  v252 = ROL(ArrayOfIntegers[8] + (v249 ^ (v251 | ~v247)) + v245 + 0x6FA57FFF, 6);
  v253 = v251 + v252;
  v254 = ROL(ArrayOfIntegers[15] + (v251 ^ (v253 | ~v249)) + v247 - 0xAD318B0, 10);
  v255 = v253 + v254;
  v256 = ROL(ArrayOfIntegers[6]+ (v253 ^ (v255 | ~v251)) + v249 - 0x5CFEBB5C, 15);
  v257 = v255 + v256;
  v258 = ROL(ArrayOfIntegers[13] + (v255 ^ (v257 | ~v253)) + v251 + 0x4E085011, 21);
  v259 = v257 + v258;
  v260 = ROL(ArrayOfIntegers[4] + (v257 ^ (v259 | ~v255)) + v253 - 0x8AA80CE, 6);
  v261 = v259 + v260;
  v262 = ROL(ArrayOfIntegers[11] + (v259 ^ (v261 | ~v257)) + v255 + 0x453AF385, 10);
  v263 = v261 + v262;
  v264 = ROL(ArrayOfIntegers[2] + (v261 ^ (v263 | ~v259)) + v257 + 0x2AD7830B, 15);
  v266 = v263 + v264;
  v267 = ROL(ArrayOfIntegers[9] + (v263 ^ (v266 | ~v261)) + v259 - 0x14A92DDF, 21);
  v271[0] = v261 + 0x673522B1;
  v271[1] = v267 + v266 - 0x10325537;
  v271[2] = v266 - 0x674532B2;
  v271[3] = v263 + 0x113255C6;

  v268 = a3;
  v269 = 0;
  do
  {
    ArrayOfIntegers[v269] ^= v268 + v271[v269 & 3];
    v268 = ArrayOfIntegers[v269++];
  }
  while ( v269 < 20 );
  return result;
}
